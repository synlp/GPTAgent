...
def send_request(data):
    ...
    if "error" in response:
        return response
    logger.debug(str(response).strip())
...
def chitchat(messages):
    data = copy.deepcopy(DEFAULT_GPT_ARG)
...
def parse_task(context, input):
    ...
    data = copy.deepcopy(DEFAULT_GPT_ARG)
    data['messages'] = messages
    data['logit_bias'] = {item: config["logit_bias"]["parse_task"] for item in task_parsing_highlight_ids}
    return send_request(data)
...
def model_inference(model_id, data, hosted_on, task):
    # for local models, hosted_on should be "local"
...
def run_task(input, command, results):
    ...
    if task.endswith("-text-to-image") or task.endswith("-control"):
        ...
        if len(all_avaliable_model_ids) == 1:
            ...
            # this step get local model information
            cand_models_info = [
                {
                    "id": model["id"],
            ...
            # this part choose the model using OpenAI GPT
...
def chat_agent(messages, return_planning=False, return_results=False):
    start = time.time()
    context = messages[:-1]
...
def main():
    messages = []
    print("Welcome! Please enter your request (`exit` to exit).")
    while True:
        message = input("[ User ]: ")
        if message == "exit":
            break
        messages.append({"role": "user", "content": message})
        answer = chat_agent(messages, return_planning=False, return_results=False)
        print("[ Agent ]: ", answer["message"])
